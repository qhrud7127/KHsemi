...
<body>
	<%
		...
		int commentID = 0;
		if (request.getParameter("commitid") != null){
			commentID = Integer.parseInt(request.getParameter("commitid"));
		}
	%>
	...
	<div class="container">
		...
		<div class="container">
			<table class="table table-haver" style="border: 1px solid #dddddd">			
					...
					%>
							<a href = "left.jsp?update&bbsID=<%= bbsID %>" class="btn-primary">수정</a>
...
// 							commentDAO.setting(commentDAO.getAllCount());
							...
							for(int i=0; i<list.size(); i++){
								...
								<form method="post" action="commentUpdateAction.co?bbsID=<%=bbsID%>&commentID=<%=commentID%>">
									<table class="table table-striped"
										style="text-align: center; border: 1px solid #dddddd">
										<tr>
											<td style="border-bottom: none;" valign="middle"><br>
											<br><%=list.get(i).getAuthor()%></td>
											<td><input type="text" style="height: 100px;" class="form-control" value="<%=list.get(i).getCommentText()%>" name="commentText"></td>
											<td><br>
											<br>
											<input type="submit" class="btn-primary pull" value="댓글 수정"></td>
										</tr>
									</table>
								</form>
								...
								<div class="row">
									<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
										...
										<td align="right"><%
												...
												%>
														<a href="left.jsp?page=commup&commitid=<%= list.get(i).getCommentID()%>&bbsID=<%= list.get(i).getBbsID()%> "> 수정</a>
